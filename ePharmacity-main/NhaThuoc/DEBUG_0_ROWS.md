# 🔍 **DEBUG: TẠI SAO CẬP NHẬT 0 DÒNG**

## 🚨 **VẤN ĐỀ:**
Khi thực hiện hoàn trả hàng, hiển thị "Đã cập nhật 0 dòng" - có nghĩa là không tìm thấy sản phẩm trong bảng `Sanphamthuoc`.

## 🔧 **CÁCH DEBUG:**

### **Bước 1: Chạy script kiểm tra mapping**
```sql
-- Chạy file: CHECK_MAPPING.sql
-- Script này sẽ kiểm tra:
-- 1. Tất cả Masanpham trong Chitietdonhang
-- 2. Tất cả Mahanghoa trong Sanphamthuoc
-- 3. Mapping giữa hai bảng
...
2. **"Tìm thấy sản phẩm: [Tenhang], Kho hiện tại: [Số lượng]"** HOẶC **"KHÔNG TÌM THẤY sản phẩm với Mahanghoa = [Masanpham]"**
3. **"Đã cập nhật [Số dòng] dòng cho sản phẩm [Masanpham]"**

### **Bước 4: Phân tích kết quả**

#### **Nếu có thông báo "KHÔNG TÌM THẤY sản phẩm":**
- **Vấn đề:** `Masanpham` trong `Chitietdonhang` không khớp với `Mahanghoa` trong `Sanphamthuoc`
- **Nguyên nhân:** Dữ liệu không đồng bộ giữa hai bảng
- **Giải pháp:** Cần sửa dữ liệu để đồng bộ

#### **Nếu có thông báo "Tìm thấy sản phẩm" nhưng vẫn "Đã cập nhật 0 dòng":**
...
- **Nguyên nhân:** Có thể là vấn đề về cache hoặc hiển thị
...
3. **Kết quả của script CHECK_MAPPING.sql là gì?**
